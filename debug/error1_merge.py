...
TARGET_PATH = os.getenv('TARGET_PA　TH', r'G:\共有ドライブ\★OD_管理者\データマネジメント部\DataOps\オペレーション\商品管理\test_data')
...
def process_file(file_path, master_headers):
  ...
  for row in data[header_row_index+1:]:
    ...
    for header in master_headers:
      # 列の位置はソース側ヘッダー配列内のインデックスで決め、C列以降なので2を加算する
...
def process_phase5():
  ...
  files = [f for f in os.listdir(base_dir) if f.startswith("PAT") and f.endswith("_normalized.xlsx") and not f.endswith("_normalized_normalized.xlsx")]
